<template>
  <BaseModal :is-open="isOpen" title="AI Marketing Copy Generator" @close="$emit('close')">
    <div class="space-y-4">
      ...
      <div class="space-y-3">
        <div>
          <label class="block text-xs font-medium text-zinc-300 mb-1">Product or Course Name</label>
          <input
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-medium text-zinc-300 mb-1">Brand Tone</label>
            <select
              v-model="form.tone"
              class="ui-input w-full px-3.5 py-2 text-xs"
            >
          ...
          <div>
            <label class="block text-xs font-medium text-zinc-300 mb-1">Target Persona</label>
            <input
      ...
      <div v-if="result" class="mt-5 pt-4 border-t border-zinc-800 space-y-3">
        <div class="p-4 bg-zinc-900/80 border border-zinc-800 rounded-xl space-y-3">
          <div>
            ...
            <h4 class="text-sm font-bold text-white mt-0.5">{{ result.headline }}</h4>
          </div>

          <div>
            <span class="text-[10px] font-bold uppercase text-brand-400 tracking-wider">Subheadline</span>
            <p class="text-xs text-zinc-300 mt-0.5 leading-relaxed">{{ result.subheadline }}</p>
          </div>
